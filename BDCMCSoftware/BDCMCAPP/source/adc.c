...
void ADCInit(void)
{
    ...
    // Bits 9-8 - 00    Unsigned Integer Fractional Output is 10
...
void __attribute__((interrupt, no_auto_psv)) _DMA5Interrupt(void)
{
    ...
    if(ADCCurrentDMABuffer == 0)
    {
        ...
        {
            vref += ADCBufferA.ADC1CH0[i];
            current += ADCBufferA.ADC1CH1[i];
        }
    }
    else
    {
        for(i = 0; i < ADC_DMA_BUFFER_SIZE; i++)
        {
            vref += ADCBufferB.ADC1CH0[i];
            current += ADCBufferB.ADC1CH1[i];
        }
    }

    if(hMotorData)
    {
        hMotorData->VRail = vref / ADC_DMA_BUFFER_SIZE;
        hMotorData->Current = current / ADC_DMA_BUFFER_SIZE;
    }
