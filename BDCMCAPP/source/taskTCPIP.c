...
void xTCPIPTaskInit(void)
{
    ...
    gUDPData.Socket = INVALID_UDP_SOCKET;
...
CONFIG_DEFAULT_UDP:
    ...
    gUDPConfig.ControllerIP = AppConfig.DefaultIPAddr;  // Default to self in charge for safety
...
static void UDPRXHandler(void)
{
    ...
    if(UDPSocketInfo[gUDPData.Socket].remoteNode.IPAddr.byte.MB == gCommonMsgData.ControllerAdd)
    {
