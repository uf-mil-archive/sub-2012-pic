////////////////////////////////////////////////////////////////////////////////
// I2C Actuator Board Controller (source)               Authored by:   Paul Launier
////////////////////////////////////////////////////////////////////////////////
...
unsigned int jDone;
I2C_DRV i2cfan = I2C_DRV_DEFAULTS;
I2C_DRV i2cActuator = I2C_DRV_DEFAULTS;
I2C_DATA fanData;
I2C_DATA actuatorData;
I2C_DATA i2cData;
	...
	if((I2C1STATbits.R_W == 0)&&(I2C1STATbits.D_A == 0))	//Address matched
        {
            ...
            Flag.AddrFlag = 1;	//next byte will the location to store data
        }
	else if((I2C1STATbits.R_W == 0)&&(I2C1STATbits.D_A == 1))	//check for data
        {
            if(Flag.AddrFlag)
            {
                ...
                Temp = I2C1RCV;         //do nothing
            }
            else if(Flag.DataFlag)
            {
                actuators = (UINT8)I2C1RCV; //store data
        ...
        {
            Temp = I2C1RCV;
            I2C1TRN = limitSwitch&1;	//send data to I2C master device
...
void updateActuatorStates(I2C_DRV *i2c){

    //write out acctuator states
    i2c->cmd = I2C_WRITE;

    i2c->oData = &actuatorData;
    i2c->oData->addr = ACTUATOR_ADDR;
    i2c->oData->reg = 0;
    //i2c->oData->buff = actuators;
	i2c->oData->buff = 0xFF;		//temp for debugging

    while(i2c->cmd!=I2C_IDLE && i2c->cmd != I2C_ERR){
        I2Cdrv(i2c);
    }
}

unsigned int getLimitSwitchState(I2C_DRV *i2c){

    //read back limit state from accuator
    //this function is used by the master
    i2c->cmd = I2C_READ;
    i2c->oData = &actuatorData;
    i2c->oData->addr = ACTUATOR_ADDR;
    i2c->oData->reg = 0;
    i2c->oData->buff = 0b000000;        //will be over written with the read
...
void FanFullOn(I2C_DRV *i2c){
        ...
        i2c->oData->buff = 0b000010;    // FULLON, 5 Volts, prescaler = 4
...
void FanFullOff(I2C_DRV *i2c){
        ...
        i2c->oData->addr = FANCNTRL_ADDR;
        i2c->oData->reg = FANCON;
        i2c->oData->buff = 0xF;    // FULLOFF, 5 Volts, prescaler = 4
...
//    IPC4bits.SI2C1IP = 7;   // Set interrupt priority to 6
    I2C1CONbits.I2CEN=1;    //enable the i2C module
        ...
        IEC1bits.SI2C1IE = 1;   //Enable I2C1 Slave Event Interrupts
        IFS1bits.SI2C1IF = 0;   //Clear I2C1 Interupt Flag
    #endif
