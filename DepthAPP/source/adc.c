...
static MAVGFilter vpresMAVG;
static MAVGFilter vtempMAVG;
...
void __attribute__((interrupt, no_auto_psv)) _DMA5Interrupt(void)
{
    ...
    if(!adcMAVGsInited)
    {
        ...
        {
            MAVG_Init(&vpresMAVG, &ADCBufferA.ADC1CH0[0], ADC_MAVG_QUEUE_LENGTH);
            MAVG_Init(&vtempMAVG, &ADCBufferA.ADC1CH1[0], ADC_MAVG_QUEUE_LENGTH);
        }
        else
        {
            MAVG_Init(&vpresMAVG, &ADCBufferB.ADC1CH0[0], ADC_MAVG_QUEUE_LENGTH);
            MAVG_Init(&vtempMAVG, &ADCBufferB.ADC1CH1[0], ADC_MAVG_QUEUE_LENGTH);
        }
        adcMAVGsInited = TRUE;
        goto ADC_DONE;
    }
    ...
    if(ADCCurrentDMABuffer == 0)
    {
        MAVG_Update(&vpresMAVG, &ADCBufferA.ADC1CH0[0], ADC_DMA_BUFFER_SIZE);
        MAVG_Update(&vtempMAVG, &ADCBufferA.ADC1CH1[0], ADC_DMA_BUFFER_SIZE);
    }
    else
    {
        MAVG_Update(&vpresMAVG, &ADCBufferB.ADC1CH0[0], ADC_DMA_BUFFER_SIZE);
